...
def index(request):
    student_no = request.GET.get('student_no', '').strip()
    student_name = request.GET.get('student_name', '').strip()
    job_name = request.GET.get('job_name', '').strip()
    selected_targets_str = request.GET.get('selected_targets_str', '').strip()
    custom_config_content = request.GET.get('custom_config_content', '').strip()
    custom_selected_targets_str = request.GET.get('custom_selected_targets_str', '').strip()

    # 构建查询条件
    queries = Q()

    if student_no:
        queries &= Q(student_no=student_no)
    if student_name:
        queries &= Q(student_name=student_name)
    if job_name:
        queries &= Q(job_name=job_name)
    if selected_targets_str:
        queries &= Q(selected_targets_str=selected_targets_str)
    if custom_config_content:
        queries &= Q(custom_config_content=custom_config_content)
    if custom_selected_targets_str:
        queries &= Q(custom_selected_targets_str=custom_selected_targets_str)

    students = Student.objects.filter(queries).order_by('-id')
    ...
    return render(request, 'student/index.html', {
        ...
        'job_name': job_name,
        'selected_targets_str': selected_targets_str,
